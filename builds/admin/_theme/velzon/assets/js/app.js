...
(function () {
    ...
    function isLoadBodyElement() {
        var verticalOverlay = document.querySelectorAll(".vertical-overlay");
    ...
    function windowLoadContent() {
        ...
        document.addEventListener("DOMContentLoaded", function () {
            var checkbox = document.querySelectorAll(".code-switcher");
    ...
    function initActiveMenu() {
        var currentPath = location.pathname == "/" ? "index.do" : "/" + location.pathname.substring(1); // a 태그의 href 값이 전체 경로 일때
        //currentPath = currentPath.substring(currentPath.lastIndexOf("/") + 1); a 태그의 href 값이 파일명만 일때
        if (currentPath) {
            // navbar-nav
            var a = document.getElementById("navbar-nav").querySelector('[href="' + currentPath + '"]');
            if (a) {
    ...
    /*function searchDetailButton() {
        const openButton = document.querySelector('#search_detail_button');
        if ( openButton ) {
            const icon = openButton.querySelector('.ri-add-circle-line');
    ...
    function searchDetail() {
        ...
        const status = JSON.parse(localStorage.getItem('search_detail')) === null ? {} : JSON.parse(localStorage.getItem('search_detail'));
        const $target = document.querySelector('#search_detail');
        const $checkbox = document.querySelector('[data-collapse-checkbox]');
        const pathname = window.location.pathname.split('/')[2];
        const object = {};
        object[pathname] = 'show';
        
        if (!$checkbox) return false;
        
        if (status !== null) {
            if (status[pathname] !== undefined && status[pathname] !== null && status[pathname] === 'show') {
        ...
        $checkbox.addEventListener('change', e => {
            if (e.target.checked) {
                Object.assign(status, object);
                localStorage.setItem('search_detail', JSON.stringify(status));
            } else {
                delete status[pathname];
                localStorage.setItem('search_detail', JSON.stringify(status));
            }
        });
    }
    
    function daumPostcode($postcode, $line1, $line2) {
        new daum.Postcode({
    ...
    function formAddress() {
        const $box = document.querySelectorAll('[data-address="box"]');
        
        if ($box.length === 0) return false;
        
        document.addEventListener('click', e => {
            if ( e.target.getAttribute('data-address') === 'search' ) {
                const $box = e.target.closest('[data-address="box"]');
                const $postcode = $box.querySelector('[data-address="postcode"]');
                const $line1 = $box.querySelector('[data-address="line1"]');
                const $line2 = $box.querySelector('[data-address="line2"]');
    
                daumPostcode($postcode, $line1, $line2);
            }
        });
    }
    
    function formAddressGroup() {
        const $addButtons = document.querySelectorAll('[data-address="add"]');
    
        if ( $addButtons.length === 0 ) return false;
    
        document.addEventListener('click', e => {
            if ( e.target.getAttribute('data-address') === 'delete' ) {
        ...
        for (let i = 0; i < $addButtons.length; i++) {
            ...
            const groupName = $addButton.getAttribute('data-address-group');
            const $group = document.querySelector(`#${groupName}`);
            
            $addButton.addEventListener('click', e => {
                const $box = document.createElement('div');
                $box.setAttribute('data-address', 'box');
    
                $box.innerHTML = `<button type="button" class="btn btn-sm btn-danger" data-address="delete">삭제</button> <div class="input-group"> <input type="text" name="" class="form-control" placeholder="우편번호" data-address="postcode" style="max-width: 120px;" readonly> <button type="button" class="btn btn-dark" data-address="search">우편번호 검색</button> </div> <input type="text" name="" class="form-control mt-1" placeholder="기본주소" data-address="line1" readonly> <input type="text" name="" class="form-control mt-1" placeholder="상세주소" data-address="line2">`;
    ...
    function fileImage() {
        // const fileImages = document.querySelectorAll('.file-image');
        const fileReader = new FileReader();
        
        document.addEventListener('change', e => {
            if (e.target.matches('.file-image *')) {
                const fileImage = e.target.closest('.file-image');
                const fileImageInput = fileImage.querySelector('input[type="file"]');
                const previewImg = fileImage.querySelector('.file-image-preview img');
                const removeButton = fileImage.querySelector('[data-btn-remove]');
                const sampleButton = fileImage.querySelector('[data-btn-sample]');
                ...
                removeButton.addEventListener('click', e => {
                    ...
                    previewImg.src = '/builds/admin/_theme/velzon/assets/images/blank.svg';
