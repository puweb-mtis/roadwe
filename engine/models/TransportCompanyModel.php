...
class TransportCompanyModel extends MY_Model
{
    ...
    function query($params = null, $type = null)
    {
        ...
        if (isset($params['sec_default']) && is_bool($params['sec_default'])) {
            $where['TPCP.comm_use_yn ='] = 'Y';   // 사용여부(Y:사용중,N:미사용)
            $where['TPCP.comm_del_yn ='] = 'N';   // 삭제여부(Y:삭제됨,N:미삭제)
        }

        // 폼 조건 검색(키워드&검색어)
        if (isset($params['sec_column']) && isset($params['sec_keyword'])) {
            $this->db->like('TPCP.' . $params['sec_column'], $params['sec_keyword']);
        }
        ...
        if (isset($params['sec_st_date']) || isset($params['sec_ed_date'])) {
            ...
            if (isset($params['sec_st_date']) && isset($params['sec_ed_date'])) {
                if ($params['sec_date_type'] === 'comm_reg_date') {
                    $this->db->where("TPCP.c_datetime BETWEEN '{$params['sec_st_date']}{$stime}' AND '{$params['sec_ed_date']}{$etime}' ");
                } else {
                    $this->db->where("TPCP.m_datetime BETWEEN '{$params['sec_st_date']}{$stime}' AND '{$params['sec_ed_date']}{$etime}' ");
                }
            } elseif (isset($params['sec_st_date']) && empty($params['sec_ed_date'])) {
                if ($params['sec_date_type'] === 'comm_reg_date') {
                    $where['TPCP.c_datetime >='] = "{$params['sec_st_date']}{$stime}";
                } else {
                    $where['TPCP.m_datetime >='] = "{$params['sec_st_date']}{$stime}";
                }
            } elseif (empty($params['sec_st_date']) && isset($params['sec_ed_date'])) {
                if ($params['sec_date_type'] === 'comm_reg_date') {
                    $where['TPCP.c_datetime <='] = "{$params['sec_ed_date']}{$etime}";
                } else {
                    $where['TPCP.m_datetime <='] = "{$params['sec_ed_date']}{$etime}";
                }
            }
        }

        // 조건 검색(운송회사 정보 일련번호)
        if (isset($params['tpcp_idx'])) {
            $where['TPCP.id'] = $params['tpcp_idx'];
        }
        return $where;
    }
