 ...
 * 일괄변경 항목
 * 1) [회원] -> [변경텍스트명]
 * 2) [member] -> [해당로직명] -> class명 첫글자 대문자로 변경
 * 3) [acmb_] -> [driving 테이블의 AS선언명]
 */
class Member extends CI_Controller
{
    // private $idx_column_nm = 'spmg_idx';            // 회원 테이블 일련번호
    private $idx_column_nm = 'id';                  // 회원 테이블 일련번호
    ...
    public function formVars()
    {
        // 회원 공통 변수 선언
        $this->data = [
            'acmb_member_type_cd' => $this->meta->get('acmb_member_type_cd', 10), // 회원 접근유형
            // 'spmg_status_cd' => $this->meta->get('spmg_status_cd', 10), // 회원 접근승인여부
        ];
    ...
    public function index()
    {
        ...
        $sec_params['sec_member_type'] = ifEmpty($sec_params,'sec_member_type');
        $sec_params['sec_has_car'] = ifEmpty($sec_params,'sec_has_car');
        $sec_params['sec_is_approve'] = ifEmpty($sec_params,'sec_is_approve');
        ...
        $sec_params['sec_default_use_status'] = true;
        ...
        if (!isset($sec_params['od_column']) && !isset($sec_params['od_type'])) {
            ...
            // $sec_params['od_column'] = 'ACMB.comm_reg_date';
            $sec_params['od_column'] = 'ACMB.c_datetime';
        }
        ...
        if ($this->siteinit->_request_type === 'ajax') {
            ...
            $sec_params['limit'] = isset($sec_params['limit']) ? $sec_params['limit'] : 10;
        }

        $data['cntall'] = $this->memberModel->selectCountAll();
        // 커버링 인덱스 쿼리 사용
        // $data_list = $this->memberModel->coveringindexActiveList($sec_params);
        // $data['cnt'] = $data_list['rows'];
        // $data['list'] = $data_list['list'];
        // 커버링 인덱스 쿼리 미사용
        $data['cnt'] = $this->memberModel->selectCount($sec_params);
        $data['list'] = $this->memberModel->selectList($sec_params);
        ...
        // 회원 요약정보
        // cnt_total, cnt_pu, cnt_rm, cnt_dv
        $data['member_statistics'] = $this->memberModel->getMemberStatistics();

        // 화면 공통 변수 호출
        $this->formVars();
        // 추가 스크립트 선언
        $this->scripts[] = $this->common->scripts(array('member.js'));
        // 화면 뷰
    ...
    /**
     * 기본(회원) 등록&수정 폼입니다.(등록/수정 폼을 같이 쓸 경우)
     * @param int $idx 회원 일련번호
     */
    public function form($idx = null)
    {
        ...
        if (is_null($idx)) { // 등록화면
            ...
            try {
                ...
                $data['row'] = $this->memberModel->selectRow($where);
                unset($where);

                if ($data['row']) {
                    // 수정 form 형식에 맞게 데이터 변환이 필요한 경우 처리
                    // ex) 이메일/전화&팩스번호/휴대폰번호 배열 변환
    ...
    public function formData($crud_data)
    {
        ...
        if ($this->uri->rsegment(3) === 'create') {
            ...
            // 회원 고유번호 생성
            $crud_data['member_code'] = $this->uniquecodegenerator->getCodeOfMember();
        }
    ...
    public function crudProc($crud_mode = null, $idx = null)
    {
        if ($crud_mode === 'create' || $crud_mode === 'update') { // 등록 && 수정
            ...
            // 폼 값 서버단 검증 처리
            $form_validation_rules = ($crud_mode === 'create') ? 'member_create' : 'member_update';
            if ($this->form_validation->run($form_validation_rules)) {
                $crud_data = $this->formData($crud_data);

                try {
                    if ($crud_mode === 'create') { // 등록
                        $acmb_key = $this->memberModel->insert($crud_data);
                    } elseif ($crud_mode === 'update') { // 수정
                        if (isset($crud_data['passwd_chg'])) { // 비밀번호 변경 체크인 경우
                            if (!empty($crud_data['password']) && !empty($crud_data['re_password'])) { // 비밀번호&비밀번호확인 값이 있는 경우
                                if ($crud_data['password'] === $crud_data['re_password']) { // 비밀번호&비밀번호확인 값이 동일한 경우
                                    // if ($this->siteinit->_admin_info['acmg_pw'] === makePasswd($crud_data['admin_password'])) {
                                    //     // 패스워드
                                    //     $crud_data['spmg_pw'] = makePasswd($crud_data['spmg_pw']);
                                    // } else {
                                    //     $this->js->pageBack($this->lang->line('cm_member_passwd_admin_error'));
                                    //     return false;
                                    // }

                                    $crud_data['password'] = makePasswd($crud_data['password']);
                                } else {
                                    $this->js->pageBack($this->lang->line('cm_member_passwd_matches_error'));
                                    return false;
                                }
                            } else {
                                $this->js->pageBack($this->lang->line('cm_member_passwd_valid_error'));
                                return false;
                            }
                        }

                        $update_where[$this->idx_column_nm] = $idx;
                        $acmb_key = $this->memberModel->update($crud_data, $update_where);
                        unset($update_where);
                    }

                    if ($acmb_key) {
                        if ($crud_mode === 'create') { // 등록
                            ajaxExcep("cm_member_{$crud_mode}_success", site_url(ADMINURL . '/member/index'));
                            // $this->js->pageRedirect(site_url(ADMINURL . '/member/index'), $this->lang->line("cm_member_{$crud_mode}_success"));
                            // return false;
                        } else { // 수정
                            ajaxExcep("cm_member_{$crud_mode}_success", $this->input->post('referer'));
                            // $this->js->pageRedirect($this->input->post('referer'), $this->lang->line("cm_member_{$crud_mode}_success"));
                            // return false;
                        }
                    } else {
                        throw new Exception($this->lang->line("cm_member_{$crud_mode}_error"));
                    }
                } catch (Exception $exc) {
                    $this->js->pageBack($exc->getMessage());
                    return false;
            ...
            $update_res = $this->memberModel->update($update_data, $update_where);
            unset($update_data);
            unset($update_where);

            ajaxExcep('cm_member_update_success', null, true);
            // json 타입 리턴일 경우 사용
            // $json['result'] = true;
            // $json['msg'] = $this->lang->line('cm_member_update_success');
            ...
            try {
                ...
                if (isset($ary_del_idx)) {
                    ...
                    $this->memberModel->updateBatch($update_data);

                    ajaxExcep('cm_member_delete_success', null, true);
                    // json 타입 리턴일 경우 사용
                    // $json['result'] = true;
                    // $json['msg'] = $this->lang->line('cm_member_delete_success');
                    // echo json_encode($json);
                    // return;
                } else {
                    throw new Exception($this->lang->line('cm_have_seq_data_error'));
            ...
            if ($this->uri->rsegment(4)) {
                try {
                    ...
                    if ($data['row']) {
                        ...
                        $this->memberModel->delete($where);
                        unset($where);

                        ajaxExcep("cm_member_{$crud_mode}_success", $this->input->server('HTTP_REFERER'));
                        // $this->js->pageRedirect($this->input->server('HTTP_REFERER'), $this->lang->line("cm_member_{$crud_mode}_success"));
                        // return false;
                    } else {
                        throw new Exception($this->lang->line('cm_have_seq_data_error'));
                    }
                ...
                ajaxExcep("cm_member_{$crud_mode}_error", site_url(ADMINURL . '/member/index'));
                // $this->js->pageRedirect(site_url(ADMINURL . '/member/index'), $this->lang->line("cm_member_{$crud_mode}_error"));
                // return false;
            }            
        } else {
