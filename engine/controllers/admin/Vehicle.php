 ...
 * 일괄변경 항목
 * 1) [차량] -> [변경텍스트명]
 * 2) [vehicle] -> [해당로직명] -> class명 첫글자 대문자로 변경
 * 3) [vdif_] -> [driving 테이블의 AS선언명]
 */
class Vehicle extends CI_Controller
{
    private $idx_column_nm = 'id';            // 차량 테이블 일련번호
    ...
    function __construct()
    {
        ...
        // 운전자 단말기 모델 호출
        $this->load->model('deviceDriverModel');
        // 차종 모델 호출
        $this->load->model('carModel');
        // 운전기사 모델 호출
        $this->load->model('driverModel');
        // 운송회사 정보 모델 호출
        $this->load->model('transportCompanyModel');
    }
    ...
    public function formVars()
    {
        ...
        // 차량 공통 변수 선언
        $this->data = [
            'device_driver_list' => $this->deviceDriverModel->selectList($where), // 운전자 단말기 리스트
            'car_model_list' => $this->carModel->selectList($where), // 차종 리스트
            'driver_list' => $this->driverModel->selectList($where), // 운전기사 리스트
            'transport_company_list' => $this->transportCompanyModel->selectList($where), // 운송회사 정보 리스트
        ];
        // debug_var($this->data);
    ...
    public function index()
    {
        ...
        $sec_params['sec_car_owner_type_cd'] = ifEmpty($sec_params, 'sec_car_owner_type_cd');
        ...
        if (!isset($sec_params['od_column']) && !isset($sec_params['od_type'])) {
            ...
            $sec_params['od_column'] = 'VDIF.c_datetime';
        }
        ...
        $data['cntall'] = $this->vehicleModel->selectCountAll();
        // 커버링 인덱스 쿼리 사용
        // $data_list = $this->vehicleModel->coveringindexActiveList($sec_params);
        // $data['cnt'] = $data_list['rows'];
        // $data['list'] = $data_list['list'];
        // 커버링 인덱스 쿼리 미사용
        $data['cnt'] = $this->vehicleModel->selectCount($sec_params);
        $data['list'] = $this->vehicleModel->selectList($sec_params);
        ...
        $this->formVars();
        // 추가 스크립트 선언
        $this->scripts[] = $this->common->scripts(array('vehicle.js'));
        // 화면 뷰
    ...
    /**
     * 기본(차량) 등록&수정 폼입니다.(등록/수정 폼을 같이 쓸 경우)
     * @param int $idx 차량 일련번호
     */
    public function form($idx = null)
    {
        ...
        if (is_null($idx)) { // 등록화면
            ...
            // row 초기 값 선언
            $data['row'] = $this->vehicleModel->listFields();
        } else { // 수정화면
            try {
                $data['crud_mode'] = 'update';
                $where[$this->idx_column_nm] = $idx;
                $data['row'] = $this->vehicleModel->selectRow($where);
                unset($where);
    ...
    public function formData($crud_data)
    {
        ...
        // 차량 소독여부
        $crud_data['disinfection_status'] = (isset($crud_data['disinfection_status']) && $crud_data['disinfection_status'] === 'on') ? 'YES' : 'NO';
    ...
    public function crudProc($crud_mode = null, $idx = null)
    {
        if ($crud_mode === 'create' || $crud_mode === 'update') { // 등록 && 수정
            ...
            // 폼 값 서버단 검증 처리
            $form_validation_rules = ($crud_mode === 'create') ? 'vehicle_create' : 'vehicle_update';
            if ($this->form_validation->run($form_validation_rules)) {
                $crud_data = $this->formData($crud_data);

                try {
                    if ($crud_mode === 'create') { // 등록
                        $vdif_key = $this->vehicleModel->insert($crud_data);
                    } elseif ($crud_mode === 'update') { // 수정
                        $update_where[$this->idx_column_nm] = $idx;
                        $vdif_key = $this->vehicleModel->update($crud_data, $update_where);
                        unset($update_where);
                    }

                    if ($vdif_key) {
                        if ($crud_mode === 'create') { // 등록
                            ajaxExcep("cm_vehicle_{$crud_mode}_success", site_url(ADMINURL . '/vehicle/index'));
                            // $this->js->pageRedirect(site_url(ADMINURL . '/vehicle/index'), $this->lang->line("cm_vehicle_{$crud_mode}_success"));
                            // return false;
                        } else { // 수정
                            ajaxExcep("cm_vehicle_{$crud_mode}_success", $this->input->post('referer'));
                            // $this->js->pageRedirect($this->input->post('referer'), $this->lang->line("cm_vehicle_{$crud_mode}_success"));
                            // return false;
                        }
                    } else {
                        throw new Exception($this->lang->line("cm_vehicle_{$crud_mode}_error"));
                    }
                } catch (Exception $exc) {
                    $this->js->pageBack($exc->getMessage());
                    return false;
            ...
            $update_res = $this->vehicleModel->update($update_data, $update_where);
            unset($update_data);
            unset($update_where);

            ajaxExcep('cm_vehicle_update_success', null, true);
            // json 타입 리턴일 경우 사용
            // $json['result'] = true;
            // $json['msg'] = $this->lang->line('cm_vehicle_update_success');
            ...
            try {
                ...
                if (isset($ary_del_idx)) {
                    ...
                    $this->vehicleModel->updateBatch($update_data);

                    ajaxExcep('cm_vehicle_delete_success', null, true);
                    // json 타입 리턴일 경우 사용
                    // $json['result'] = true;
                    // $json['msg'] = $this->lang->line('cm_vehicle_delete_success');
                    // echo json_encode($json);
                    // return;
                } else {
                    throw new Exception($this->lang->line('cm_have_seq_data_error'));
            ...
            if ($this->uri->rsegment(4)) {
                try {
                    ...
                    if ($data['row']) {
                        ...
                        $this->vehicleModel->delete($where);
                        unset($where);

                        ajaxExcep("cm_vehicle_{$crud_mode}_success", $this->input->server('HTTP_REFERER'));
                        // $this->js->pageRedirect($this->input->server('HTTP_REFERER'), $this->lang->line("cm_vehicle_{$crud_mode}_success"));
                        // return false;
                    } else {
                        throw new Exception($this->lang->line('cm_have_seq_data_error'));
                    }
                ...
                ajaxExcep("cm_vehicle_{$crud_mode}_error", site_url(ADMINURL . '/vehicle/index'));
                // $this->js->pageRedirect(site_url(ADMINURL . '/vehicle/index'), $this->lang->line("cm_vehicle_{$crud_mode}_error"));
                // return false;
            }
        } else {
