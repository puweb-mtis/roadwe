 ...
 * 3) [vdrr_] -> [driving 테이블의 AS선언명]
 */
class Runroute extends CI_Controller
{
    private $idx_column_nm = 'id';            // 운행노선 테이블 일련번호
    ...
    public function index()
    {
        ...
        if (!isset($sec_params['od_column']) && !isset($sec_params['od_type'])) {
            ...
            $sec_params['od_column'] = 'VDRR.c_datetime';
        }
        ...
        $data['cntall'] = $this->runRouteModel->selectCountAll();
        // 커버링 인덱스 쿼리 사용
        // $data_list = $this->runRouteModel->coveringindexActiveList($sec_params);
        // $data['cnt'] = $data_list['rows'];
        // $data['list'] = $data_list['list'];
        // 커버링 인덱스 쿼리 미사용
        $data['cnt'] = $this->runRouteModel->selectCount($sec_params);
        $data['list'] = $this->runRouteModel->selectList($sec_params);

        $diff_day_ko = array("일", "월", "화", "수", "목", "금", "토");
        foreach ($data["list"] as $row => $rdata) {
            // 한글 요일 가져오기
            $data["list"][$row]["driving_week_txt"] = implode(',', getDateFromBit($rdata['driving_week']));
            $find_day_ko = getDateFromBit($rdata['driving_week']);
            $data["list"][$row]["stop_week_txt"] = implode(',', array_diff($diff_day_ko, $find_day_ko));
            unset($find_day_ko);
        }
    ...
    /**
     * 기본(운행노선) 등록&수정 폼입니다.(등록/수정 폼을 같이 쓸 경우)
     * @param int $idx 운행노선 일련번호
     */
    public function form($idx = null)
    {
        ...
        if (is_null($idx)) { // 등록화면
            ...
            // row 초기 값 선언
            $data['row'] = $this->runRouteModel->listFields();
        } else { // 수정화면
            try {
                $data['crud_mode'] = 'update';
                $where[$this->idx_column_nm] = $idx;
                $data['row'] = $this->runRouteModel->selectRow($where);
                unset($where);
    ...
    public function crudProc($crud_mode = null, $idx = null)
    {
        if ($crud_mode === 'create' || $crud_mode === 'update') { // 등록 && 수정
            ...
            if ($this->form_validation->run($form_validation_rules)) {
                ...
                try {
                    if ($crud_mode === 'create') { // 등록
                        $vdrr_key = $this->runRouteModel->insert($crud_data);
                    } elseif ($crud_mode === 'update') { // 수정
                        $update_where[$this->idx_column_nm] = $idx;
                        $vdrr_key = $this->runRouteModel->update($crud_data, $update_where);
                        unset($update_where);
                    }
            ...
            $update_res = $this->runRouteModel->update($update_data, $update_where);
            unset($update_data);
            unset($update_where);
            ...
            try {
                ...
                if (isset($ary_del_idx)) {
                    ...
                    $this->runRouteModel->updateBatch($update_data);
            ...
            if ($this->uri->rsegment(4)) {
                try {
                    ...
                    if ($data['row']) {
                        ...
                        $this->runRouteModel->delete($where);
                        unset($where);
    ...
    public function loadData($req_type = null)
    {
        ...
        if ($req_type === 'car') { //운행노선 정보를 가져온다.
            // 해당 테이블 단일행 가져오기
            // $where['id'] = $this->input->post('vdrr_idx');
            // $result_data = $this->runRouteModel->selectRow($where);

            // 해당 쿼리 데이터 가져오기
            $where['vdrr_idx'] = $this->input->post('vdrr_idx');
            $result_list = $this->runRouteModel->selectList($where);
