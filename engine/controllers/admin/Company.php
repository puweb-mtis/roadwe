 ...
 * 3) [cp_] -> [driving 테이블의 AS선언명]
 */
class Company extends CI_Controller
{
    private $idx_column_nm = 'id';            // 거래처 테이블 일련번호
    ...
    public function index()
    {
        ...
        if (!isset($sec_params['od_column']) && !isset($sec_params['od_type'])) {
            ...
            $sec_params['od_column'] = 'CP.c_datetime';
        }
        ...
        $data['cntall'] = $this->companyModel->selectCountAll();
        // 커버링 인덱스 쿼리 사용
        // $data_list = $this->companyModel->coveringindexActiveList($sec_params);
        // $data['cnt'] = $data_list['rows'];
        // $data['list'] = $data_list['list'];
        // 커버링 인덱스 쿼리 미사용
        $data['cnt'] = $this->companyModel->selectCount($sec_params);
        $data['list'] = $this->companyModel->selectList($sec_params);
        ...
        $this->formVars();
        // 추가 스크립트 선언
        $this->scripts[] = $this->common->scripts(array('company.js'));
        // 화면 뷰
    ...
    /**
     * 기본(거래처) 등록&수정 폼입니다.(등록/수정 폼을 같이 쓸 경우)
     * @param int $idx 거래처 일련번호
     */
    public function form($idx = null)
    {
        ...
        if (is_null($idx)) { // 등록화면
            ...
            // row 초기 값 선언
            $data['row'] = $this->companyModel->listFields();
        } else { // 수정화면
            try {
                $data['crud_mode'] = 'update';
                $where[$this->idx_column_nm] = $idx;
                $data['row'] = $this->companyModel->selectRow($where);
                unset($where);
    ...
    public function crudProc($crud_mode = null, $idx = null)
    {
        if ($crud_mode === 'create' || $crud_mode === 'update') { // 등록 && 수정
            ...
            if ($this->form_validation->run($form_validation_rules)) {
                ...
                try {
                    if ($crud_mode === 'create') { // 등록
                        $cp_key = $this->companyModel->insert($crud_data);
                    } elseif ($crud_mode === 'update') { // 수정
                        $update_where[$this->idx_column_nm] = $idx;
                        $cp_key = $this->companyModel->update($crud_data, $update_where);
                        unset($update_where);
                    }
            ...
            $update_res = $this->companyModel->update($update_data, $update_where);
            unset($update_data);
            unset($update_where);
            ...
            try {
                ...
                if (isset($ary_del_idx)) {
                    ...
                    $this->companyModel->updateBatch($update_data);
    ...
    public function loadData($req_type = null)
    {
        ...
        if ($req_type === 'car') { //거래처 정보를 가져온다.
            // 해당 테이블 단일행 가져오기
            // $where['id'] = $this->input->post('cp_idx');
            // $result_data = $this->companyModel->selectRow($where);

            // 해당 쿼리 데이터 가져오기
            $where['cp_idx'] = $this->input->post('cp_idx');
            $result_list = $this->companyModel->selectList($where);
