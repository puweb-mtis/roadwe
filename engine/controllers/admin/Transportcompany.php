 ...
 * 3) [tpcp_] -> [driving 테이블의 AS선언명]
 */
class Transportcompany extends CI_Controller
{
    private $idx_column_nm = 'id';            // 운송회사 테이블 일련번호
    ...
    public function index()
    {
        ...
        if (!isset($sec_params['od_column']) && !isset($sec_params['od_type'])) {
            ...
            $sec_params['od_column'] = 'TPCP.c_datetime';
        }
        ...
        $data['cntall'] = $this->transportCompanyModel->selectCountAll();
        // 커버링 인덱스 쿼리 사용
        // $data_list = $this->transportCompanyModel->coveringindexActiveList($sec_params);
        // $data['cnt'] = $data_list['rows'];
        // $data['list'] = $data_list['list'];
        // 커버링 인덱스 쿼리 미사용
        $data['cnt'] = $this->transportCompanyModel->selectCount($sec_params);
        $data['list'] = $this->transportCompanyModel->selectList($sec_params);
        ...
        $this->formVars();
        // 추가 스크립트 선언
        $this->scripts[] = $this->common->scripts(array('transportcompany.js'));
        // 화면 뷰
    ...
    /**
     * 기본(운송회사) 등록&수정 폼입니다.(등록/수정 폼을 같이 쓸 경우)
     * @param int $idx 운송회사 일련번호
     */
    public function form($idx = null)
    {
        ...
        if (is_null($idx)) { // 등록화면
            ...
            // row 초기 값 선언
            $data['row'] = $this->transportCompanyModel->listFields();
        } else { // 수정화면
            try {
                $data['crud_mode'] = 'update';
                $where[$this->idx_column_nm] = $idx;
                $data['row'] = $this->transportCompanyModel->selectRow($where);
                unset($where);
    ...
    public function crudProc($crud_mode = null, $idx = null)
    {
        if ($crud_mode === 'create' || $crud_mode === 'update') { // 등록 && 수정
            ...
            if ($this->form_validation->run($form_validation_rules)) {
                ...
                try {
                    if ($crud_mode === 'create') { // 등록
                        $tpcp_key = $this->transportCompanyModel->insert($crud_data);
                    } elseif ($crud_mode === 'update') { // 수정
                        $update_where[$this->idx_column_nm] = $idx;
                        $tpcp_key = $this->transportCompanyModel->update($crud_data, $update_where);
                        unset($update_where);
                    }
            ...
            $update_res = $this->transportCompanyModel->update($update_data, $update_where);
            unset($update_data);
            unset($update_where);
            ...
            try {
                ...
                if (isset($ary_del_idx)) {
                    ...
                    $this->transportCompanyModel->updateBatch($update_data);
            ...
            if ($this->uri->rsegment(4)) {
                try {
                    ...
                    if ($data['row']) {
                        ...
                        $this->transportCompanyModel->delete($where);
                        unset($where);
    ...
    public function loadData($req_type = null)
    {
        ...
        if ($req_type === 'car') { //운송회사 정보를 가져온다.
            // 해당 테이블 단일행 가져오기
            // $where['id'] = $this->input->post('tpcp_idx');
            // $result_data = $this->transportCompanyModel->selectRow($where);
            ...
            // 해당 쿼리 데이터 가져오기
            $where['tptpcp_idx'] = $this->input->post('tptpcp_idx');
            $result_list = $this->transporttransportCompanyModel->selectList($where);
            if ($result_list) { // 리스트 정보가 있는 경우
                $result_data = $result_list[0];
            }
        }
